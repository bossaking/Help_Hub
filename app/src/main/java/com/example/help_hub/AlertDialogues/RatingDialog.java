...
public class RatingDialog extends DialogFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ratingButton.setOnClickListener(v -> sendRating());
    ...
    private void sendRating() {
        ...
        FirebaseFirestore.getInstance().collection("users").document(otherUserId).get().addOnSuccessListener(documentSnapshot -> {
            ...
            FirebaseFirestore.getInstance().collection("users").document(otherUserId).update(userMap)
                    .addOnSuccessListener(unused -> {
                        ...
                        FirebaseFirestore.getInstance().collection("users").document(userId).get()
                                .addOnSuccessListener(documentSnapshot1 -> {
                                    ...
                                    FirebaseFirestore.getInstance().collection("users").document(otherUserId).collection("opinions").add(opinion)
                                            .addOnSuccessListener(documentReference -> {
                                                ratingChangedListener.ratingChanged();
                                                this.dismiss();
                                            })
                                            .addOnFailureListener(e -> Toast.makeText(getContext(), R.string.error + e.getLocalizedMessage(), Toast.LENGTH_LONG).show());
                                });
                    })
